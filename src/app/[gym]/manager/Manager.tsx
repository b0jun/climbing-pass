			...
			overlay.open(({ isOpen, close, exit }) => (
				...
				>
					<p className="text-[14px] text-black/60">
						입장 상태가
						<span className="bg-gray-100 mx-2 text-gray-800 text-xs font-medium px-2.5 py-0.5 rounded">
							대기
						</span>
						상태로 변경됩니다.
					</p>
				...
				<table className="w-full text-sm text-left rtl:text-right text-gray-500 lg:table-fixed">
						...
						<tr>
							{tableHeaderList.map((item, index) => (
								<th
									key={item}
									scope="col"
									className={cn('px-4 py-3 whitespace-nowrap last:text-right group')}
								>
					...
					<tbody>
						{data.passList.length > 0 ? (
							data.passList.map(
								(
									{ id, name, phoneNumber, dateOfBirth, type, shoesRental, status, createdAt }: any,
									index: number
								) => (
													...
													{status === 'WAIT' ? (
														<div className="relative">
															<button
																...
																onClick={() =>
																	changePassStatus({ id, status: 'APPROVED' })
																}
															>
																입장
															</button>
